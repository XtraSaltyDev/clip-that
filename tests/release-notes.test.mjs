...
test('bundles the 0.1.12 native Mac system audio release notes', () => {
  const notes = RELEASE_NOTES.find((entry) => entry.version === '0.1.12')
  assert.ok(notes)
  assert.match(notes.title, /Mac system audio/i)
  assert.ok(notes.items.some((item) => /system audio/i.test(item.title + item.body)))
  assert.match(notes.summary, /virtual audio device/i)
})
